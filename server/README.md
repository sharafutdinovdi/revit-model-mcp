...
| `REVIT_MCP_ALLOW_WRITE` | Unset | Only `1` registers the ten action tools, including `revit_batch` and `revit_align_link_datums` at server startup; the workstation gate is also required. |
