...
from revit_model_mcp.revit_channel import (
    JobPickupStatus, RevitChannelError, ResponseParseError, ResponseTimeoutError,
)
...
class HttpHost:
    def __init__(self, host: str, token: str | None = None) -> None:
        ...
        if (parsed.scheme not in {"http", "https"} or not parsed.hostname
            ...
            raise ValueError("HTTP host must be an http:// or https:// URL without credentials, query or fragment.")
    ...
    async def list_revit_instances(self, document: str | None = None) -> list[dict[str, object]]:
        ...
        if document and document.casefold() not in str(name).casefold():
            return []
        return [{
            **status, "documentTitle": name, "documentPath": "", "windowTitle": "",
    ...
    async def prepare_job(self, name: str, content: str, command: str) -> set[str]:
        ...
        status, body, headers = await self._request("POST", "/jobs?timeout=0", content.encode("utf-8"))
    ...
    async def wait_until_trigger_is_gone(self, timeout_seconds: float) -> JobPickupStatus:
        return JobPickupStatus(taken=True, activation_attempts=0, trigger_present=False, elapsed_seconds=0)
    ...
    async def wait_for_new_response(self, command: str, known_names: set[str], timeout_seconds: float) -> str | None:
        ...
        while self._response is None:
            ...
            status, body, _ = await self._request(
                "GET", "/jobs/" + urllib.parse.quote(self._job_id, safe=""), timeout=min(10, remaining)
            )
            if status == 200:
                self._response = body.decode("utf-8-sig")
                break
    ...
    async def finish_job(self, response_name: str, cleanup_names: list[str],
        ...
        if download_artifact and response.get("success") is True:
            ...
            if status != 200 or headers.get_content_type() != "image/png" or not image.startswith(b"\x89PNG\r\n\x1a\n"):
            ...
            target = Path(save_to).expanduser().resolve() if save_to else Path(tempfile.mkdtemp(prefix="revit-view-")) / name
    ...
    async def _request(self, method: str, path: str, body: bytes | None = None,
                       timeout: float = 10, authenticated: bool = True):
        headers = {"Accept": "application/json, image/png"}
        if body is not None:
            headers["Content-Type"] = "application/json"
        if authenticated:
            if not self.token or any(ord(character) < 32 or ord(character) == 127 for character in self.token):
                raise RevitChannelError("Set REVIT_MCP_TOKEN or --token to the token in the workstation settings.json.")
            headers["Authorization"] = "Bearer " + self.token
        request = urllib.request.Request(self.host + path, data=body, headers=headers, method=method)
        ...
        def send():
            try:
                ...
                raise RevitChannelError(messages.get(error.code, f"Revit endpoint returned HTTP {error.code}.")) from None
